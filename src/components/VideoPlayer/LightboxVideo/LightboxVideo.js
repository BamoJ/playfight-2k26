...
function initBunnyLightboxPlayer() {
	...
	var autoplay =
		player.getAttribute('data-player-autoplay') === 'true';
	var initialMuted =
		player.getAttribute('data-player-muted') === 'true';
	...
	var isSafariNative = !!video.canPlayType(
	...
	function setupLightboxClamp(player, wrapper, video, updateSize) {
		var calcBox = wrapper.querySelector(
			'[data-bunny-lightbox-calc-height]',
		);
		if (!calcBox) return;
	...
	function attachMediaFor(src) {
		...
		withAttach(src, function onReady() {
			...
			if (typeof player._applyClamp === 'function')
				player._applyClamp();
	...
	function ensureOpenUI(isActive) {
		...
		if (
			wrapper.getAttribute('data-bunny-lightbox-status') !== state
		) {
	...
	document.addEventListener('click', function (e) {
		...
		if (openBtn) {
			...
			if (!src) return;
			var imgEl = openBtn.querySelector(
				'[data-bunny-lightbox-placeholder]',
			);
		...
		var closeBtn = e.target.closest(
			'[data-bunny-lightbox-control="close"]',
		);
		if (closeBtn) {
			var closeInWrapper = closeBtn.closest(
				'[data-bunny-lightbox-status]',
			);
		...
		requestAnimationFrame(function () {
			...
			if (!master || master.indexOf('blob:') === 0) {
				var attrSrc =
					...
					'';
				if (attrSrc && attrSrc.indexOf('blob:') !== 0)
					master = attrSrc;
			}
