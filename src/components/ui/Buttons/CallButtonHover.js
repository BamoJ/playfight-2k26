...
export default class CallButtonHover extends ComponentCore {
	constructor() {
		super();
	...
	createElements() {
		this.items = [
			...document.querySelectorAll('.footer_credit_phone_link'),
		].map((btn) => {
			const text = btn.querySelector('.footer_credit_phone_text');
			const textHover = btn.querySelector(
				'.footer_credit_phone_text.is-abs',
			);
			...
			gsap.set(calSVG, { display: 'none', scale: 0, x: 0 });
